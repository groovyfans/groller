...
class UserController {
    ...
    def save = {
        ...
        userInstance.passwordHash = params.password // FIXEDME:
    ...
    def profile = {
        if(request.method == 'POST') {
            def userProfile = User.get(params.id)
            if (userProfile) {
                if (params.version) {
                    def version = params.version.toLong()
                    if (userProfile.version > version) {
                        
                        userProfile.errors.rejectValue("version", "user.optimistic.locking.failure", "Another user has updated this User while you were editing")
                        render(view: "profile", model: [userProfile: userProfile])
                        return
                    }
                }
                ...
                if (!params.password){
                    bindData(userProfile, params, [exclude:'password'])
                }
                else {
                    userProfile.properties = params
                }
                if (!userProfile.hasErrors() && userProfile.save()) {
                    flash.message = "user.updated"
                    flash.args = [params.id]
                    flash.defaultMessage = "User ${params.id} updated"
                     render view: 'profile', model: [userProfile: userProfile]
            ...
            def userProfile = User.findByUsername(authenticateService.principal().username)
